...
Raku tasks are high level Raku scenarios to run Sparrow tasks written on Sparrow compatible languages.
...
Show soft checks statuses in reports. By default is disabled.

* `--verbose`

Enable verbose mode, adds some low level messages.

 * `--debug`

Enable debug mode, prints even more low level messages.

* `--inline`

Enable inline mode, used when run s6 as part of Bash script:

```bash
eval $(s6 --plg-run --inline foo)
```
